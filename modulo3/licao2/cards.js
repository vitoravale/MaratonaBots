...
const CarouselCardName = 'carousel'
...
const actionWithPostback = session => builder.CardAction.postBack(session, "postbackmessage", "Botão com PostBack")
...
const create = (selected, session) => {
    switch(selected.trim().toLowerCase()){
        ...
        case CarouselCardName:
            return createCarouselCard(session)
    }
}

module.exports = {
    CardNames: [HeroCardName, ThumbnailCardName, ReceiptCardName, SigninCardName, AnimationCardName, VideoCardName, AudioCardName, CarouselCardName],
    Create: create
}