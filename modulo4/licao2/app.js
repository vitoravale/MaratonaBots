...
intents.matches('reconhecer-emocoes', (session, args, next) => {
    session.send('**ಠ~ಠ** - Desculpa, eu ainda não sei reconhecer emoções...')
})

intents.matches('descrever-imagem', (session, args, next) => {
    session.send('**(¬_¬)** - Foi mal, ainda não aprendi a descrever coisas...')
})

intents.matches('classificar-imagem', (session, args, next) => {
    session.send("**(ง'̀-'́)ง** - Quase lá... juro que na próxima vez vou saber classificar uma imagem...")
})
